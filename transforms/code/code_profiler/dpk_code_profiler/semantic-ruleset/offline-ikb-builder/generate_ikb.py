...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--null_libs_file",
        type=str,
        help="Path to null libraries file.",
        default=os.getenv("NULL_LIBS_FILE", "../ikb/null_libs.csv"),
    )
    parser.add_argument(
        "--cmap_file",
        type=str,
        help="Path to concept map file.",
        default=os.getenv("CMAP_FILE", "../concept_map/updated_concept_list.csv"),
    )
    parser.add_argument(
        "--input_examples_file",
        type=str,
        help="Path to input examples file.",
        default=os.getenv("EXAMPLES_I_FILE", "../examples/examples-i.csv"),
    )
    parser.add_argument(
        "--output_examples_file",
        type=str,
        help="Path to output examples file.",
        default=os.getenv("EXAMPLES_O_FILE", "../examples/examples-o.csv"),
    )
    parser.add_argument(
        "--extracted_data_file",
        type=str,
        help="Path to file in which LLM output will be stored.",
        default=os.getenv("EXTRACTED_DATA_FILE", "../ikb/extracted_data.csv"),
    )
    parser.add_argument(
        "--api_type",
        type=str,
        help="API Type",
        default=os.getenv("API_TYPE", "WatsonxAI"),
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help="API endpoint",
        default=os.getenv("API_ENDPOINT", "https://us-south.ml.cloud.ibm.com"),
    )
    ...
    prompt_template = (
        """You are responsible for classifying programming language packages based on their functionality into one of the following STRICT categories:
                        """
        + concepts
        + """
    ...
    for combined_string in combined_strings:
        ...
        if args.api_type == "WatsonxAI":
            response = generateResponseWatsonx(
                args.api_key,
                args.api_endpoint,
                args.model_id,
                args.project_id,
