...
class DocQualityInput(BaseModel, DPKDataAccessInput, DPKRuntimeInput):
    """Input for DocQualityTransform."""

    docq_text_lang: Optional[str] = Field(
        None,
        description="language used in the text content. By default, en is used",
    )
    docq_doc_content_column: Optional[str] = Field(
        None,
        description="column name that contain document text. By default, contents is used.",
    )
    docq_bad_word_filepath: Optional[str] = Field(
        None,
...
def add_transform_params(transform_params: dict, kwargs):
    ...
    fields = list(DocQualityInput.__annotations__.keys())
    for field in fields:
        if field in kwargs and kwargs[field] is not None:
            transform_params[field] = kwargs[field]


class DocQualityTransform(BaseTool):  # type: ignore[override, override]
    """Tool that apples doc_quality transform."""

    name: str = "doc_quality"
    args_schema: Type[BaseModel] = DocQualityInput
    description: str = "Apply DocQuality transform on files in input folder"
        ...
        **kwargs,
        ...
        try:
            ...
            if runtime_type.strip().lower() == "ray":
                ...
                return f"Error: Unrecognizable type of TransformRuntimeConfiguration  in doc_quality transform - {runtime_type}."
            print(f"launching transform with params: {transform_params}")
            return_code = launcher.launch()
            if return_code != 0:
                return "Error doc_quality Job Failed"

            return f"doc_quality transform successfully applied with input_folder {input_folder} output_folder {output_folder}."
